...
| `X-TLC-PRIORITY` | VTODO task | Mapped by configured RANK onto 1–9 (`1 + floor(rank·9/N)`; default P0–P3 → 1/3/5/7); the configured NAME is preserved in `X-TLC-PRIORITY`; empty priority emits no PRIORITY | `Task.Priority` | 0.1 (2026-09). Emitted whenever a priority is set, alongside the rank-derived numeric `PRIORITY`. Import: wins when the name is in the current vocabulary; otherwise nearest-rank fallback from `PRIORITY` |
