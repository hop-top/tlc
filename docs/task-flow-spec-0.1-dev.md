...
- `depends_on` (array of strings, optional): Step IDs that must satisfy this step before it starts
  - Default: `[]` (no dependencies)
  - If empty, step starts when flow execution reaches it
  - If non-empty, step waits until all dependencies reach `succeeded` OR `skipped` state
  - `skipped` deps unblock downstream so conditionally-gated subgraphs do not deadlock
