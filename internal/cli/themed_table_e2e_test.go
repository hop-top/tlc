...
func TestThemedTable_FourColors_E2E(t *testing.T) {
	...
	// blockerOf maps blocker ID → blocked task ID (from scenario data).
	...
	// assertVisibleColors validates coloring rules for every visible row.
	// Roles are computed dynamically based on the visible result set.
	assertVisibleColors := func(t *testing.T, out string) {
		...
		for id := range visibleSet {
			...
			isPrimary := false
			switch id {
			case "T-0001", "T-0002", "T-0004", "T-0010":
				isPrimary = true
			}
			isBlocker := false
			if dep, ok := blockerOf[id]; ok && visibleSet[dep] && !isPrimary {
				isBlocker = true
			}
			isBlocked := id == "T-0006" || id == "T-0008" || id == "T-0009"

			switch {
			case isPrimary:
				assertCellHas(t, id, "primary", cc, ansiGreen,
					[]string{ansiPink, ansiMuted})
			case isBlocker:
				assertCellHas(t, id, "blocker", cc, ansiPink,
					[]string{ansiGreen, ansiMuted})
			case isBlocked:
				assertCellHas(t, id, "blocked", cc, ansiMuted,
					[]string{ansiGreen, ansiPink})
			default:
				for _, c := range cc {
					if c == ansiGreen || c == ansiPink || c == ansiMuted {
						t.Errorf("task %s (white): unexpected cell color %q", id, c)
					}
				}
			}
		}
...
// TestThemedTable_SingleFilter_NoEmphasis_E2E verifies that a single
// status filter (--status TODO) uses per-column coloring, not row
// emphasis.
func TestThemedTable_SingleFilter_NoEmphasis_E2E(t *testing.T) {
	ctx, cleanup := setupTestDir(t)
	defer cleanup()
	resetTaskFlags()
	...
	// With a single filter, no row-level emphasis. ID columns should
	// have pink, title columns should have green (per-column rules).
	for _, id := range []string{"T-0005", "T-0007"} {
		line := rowLine(out, id)
		if line == "" {
			t.Errorf("task %s not found in output", id)
			continue
		}
		cc := cellColors(line)
		hasPink := false
		...
		if !hasPink {
			t.Errorf("task %s: expected pink (ID column) in single-filter mode", id)
		}
		if !hasGreen {
			t.Errorf("task %s: expected green (title column) in single-filter mode", id)
		}
	}
}
